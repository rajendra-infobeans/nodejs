...
export const filesPayloadExit = (req, res, next) => {
  
  if (!req.files)
    return res.status(400).json({ status: "error", message: "Files missing" });
    // return res.status(400).json({ status: "error", message: req.files });
  next();
};

export const filesExtensionLimiter = (allowedFileExtension) => {
   return (req, res, next) => {
